...
//        $mydate = strtotime(date( "m",$mydate) . "/" . (date("d",$mydate)+1) . "/" . date("Y",$mydate));
    ...
    foreach ($todisp as $loc=>$ind) {
        $otherforminfo = 'UID=' . $UID . '&st=' . $_POST['starttime'] . '&en=' . $_POST['endtime'] . '&co=' . make_get_str($_POST['comment']) . '&vi=' . $_POST['vi'] . '&';
        echo "<td nowrap bgcolor=#dddddd colspan=\"" . num_signups . "\"><font size=\"-1\">";
        if (in_array($ind,explode("+",$_POST['resource']))) echo "<b>";
...
//        if ($_POST['vi'] == 'week')         $datestamp = strtotime(date( "m",$tabledate) . "/" . (date("d",$tabledate)+$dow-1) . "/" . date("Y",$tabledate) . " " . hourtotime($hour));
        ...
        for ($priority = 1; $priority <= num_signups; $priority++) {
            ...
            if (is_array($data[$priority])) {
                ...
                if ($info['endtime']%(hour_interval*60*60)>0 & date("m/d/Y",$info['endtime'])==date("m/d/Y",$datestamp)) 
                    $content .= "<center><font size=\"-1\">(end +" . date("G:i",mktime(0,$info['endtime']%(hour_interval*60*60)/60)) . ")</font></center>";
...
//CBW 7/17/2010
// - Changed $_POST to $_POST in order to be compatible with PHP 5 
